...
-   **Monitor only**: In this mode, watchtower checks for container updates, sends notifications and invokes the [pre-check/post-check hooks](https://marrrrrrrrry.github.io/watchtower/lifecycle-hooks/) on the containers but does **not** perform the update.
...
```
:::
If you wish to create a monitoring scope, you will need to [run multiple instances and set a scope for each of them](https://containrrr.dev/watchtower/running-multiple-instances/).
